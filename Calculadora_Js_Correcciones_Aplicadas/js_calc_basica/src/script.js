...
class Calculadora {
    ...
    //Error 3 - Si lo que se presiona despues de igual es un numero entonces que borre el resultado anterior e inicie una nueva operacion
        if(this.ultimoFueIgual) {
            this.valorActual = ''
            this.ultimoFueIgual = false     
        }
        //Error 1 - Limite de dígitos
        if (this.valorActual.length >= 11) return // Limita el número de dígitos a 11
  ...
  //Error 2 - Boton de porcentaje
    ...
    calcular() {
        ...
        //Error 4 - Muestra la operación completa en el display superior
        let operacionTexto
        const valor_1 = parseFloat(this.valorPrevio)
        let valor_2 = parseFloat(this.valorActual)

        if(this.esPorcentaje && !isNaN(valor_1)) {
            valor_2 = valor_1 * (valor_2 / 100)
            this.esPorcentaje = false
            //Error 4 - Muestra la operación completa en el display superior
            operacionTexto = `${this.obtenerNumero(this.valorPrevio)} ${this.operacion} ${this.obtenerNumero(this.valorActual)}%`
        } else {
        operacionTexto = `${this.obtenerNumero(this.valorPrevio)} ${this.operacion} ${this.obtenerNumero(this.valorActual)}`
        }        
        ...
        this.valorActual = resultado.toPrecision(11) // Limita a 11 dígitos de precisión
        // Limpia la operación y el valor previo
        this.operacion = undefined
        this.valorPrevio = ''
        //Error 3 - Si lo que se presiona despues de igual es un numero entonces que borre el resultado anterior e inicie una nueva operacion
        this.ultimoFueIgual = true 
        this.operacionRealizada = operacionTexto
    }
    ...
    actualizarPantalla() {
        ...
        } else if (this.ultimoFueIgual && this.operacionRealizada) { 
            this.valorPrevioTextElement.innerText = `${this.operacionRealizada}`
        } else {
